...
const { month, direction, prev, next, today: goToday } = useMonth()
  ...
  <div class="flex min-h-dvh">
    <section class="flex min-w-0 grow flex-col gap-6 px-5 pt-14 lg:gap-7 lg:px-12 lg:pt-10">
      ...
      <div class="flex items-center justify-between">
        ...
        <button type="button" class="motion-press flex size-11 items-center justify-center text-ink2 lg:hidden" aria-label="上個月" @click="prev">
          <AppIcon name="prev" />
        </button>
        <span class="font-serif text-base lg:hidden">{{ formatMonthTitle(month) }}</span>
        <button type="button" class="motion-press flex size-11 items-center justify-center text-ink2 lg:hidden" aria-label="下個月" @click="next">
          <AppIcon name="next" />
        </button>
        <div class="hidden items-center gap-1.5 lg:flex">
          <button type="button" class="motion-press flex size-11 items-center justify-center rounded-[10px] border border-line bg-card text-ink2" aria-label="上個月" @click="prev">
            <AppIcon name="prev" :size="20" />
          </button>
          <button type="button" class="motion-press flex h-11 items-center rounded-[10px] border border-line bg-card px-4 text-sm" @click="goToday">今天</button>
          <button type="button" class="motion-press flex size-11 items-center justify-center rounded-[10px] border border-line bg-card text-ink2" aria-label="下個月" @click="next">
      ...
      <Transition :name="`month-${direction}`" mode="out-in">
        <MonthCalendar :key="month" :month="month" :moods="moods" :today="today" size="md" class="lg:hidden" @select="open" />
      </Transition>
      <Transition :name="`month-${direction}`" mode="out-in">
        <MonthCalendar :key="month" :month="month" :moods="moods" :today="today" size="lg" class="hidden lg:grid" @select="open" />
      </Transition>

      <NuxtLink :to="`/entry/${today}`" class="motion-card flex items-center gap-3.5 rounded-2xl border border-line bg-card p-4 lg:hidden">
    ...
    <aside class="hidden min-h-dvh w-[400px] shrink-0 flex-col gap-5 border-l border-line bg-card px-8 py-10 lg:flex">
      ...
      <div class="grow" />
      <NuxtLink :to="`/entry/${today}`" class="motion-press flex h-11 items-center justify-center gap-2 rounded-[10px] border border-line bg-paper text-sm">
