  ...
  <div class="flex min-h-dvh flex-col gap-5 px-5 pt-14 lg:gap-7 lg:px-12 lg:pt-10">
    ...
    <div class="flex flex-col gap-5 lg:w-[640px]">
      <div class="flex gap-3">
        <div v-for="tile in [{ label: '本月寫了', value: stats.daysWritten }, { label: '連續書寫', value: stats.streak }]" :key="tile.label" class="flex flex-1 flex-col gap-1 rounded-2xl border border-line bg-card p-4">
      ...
      <div class="flex flex-col gap-3.5 rounded-2xl border border-line bg-card px-4 py-[18px]">
        ...
        <div v-for="mood in stats.moods" :key="mood.mood" class="flex h-[34px] items-center gap-3 lg:h-[38px]">
          ...
          <div class="h-2.5 grow overflow-hidden rounded bg-chip">
            <div class="h-full rounded-r bg-accent" :style="{ width: `${Math.round((mood.count / maxMoodCount) * 100)}%` }" />
          </div>
